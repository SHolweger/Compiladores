...
def p_funcion_vacia(p):
    'funcion : FUNCION IDENTIFICADOR PARENIZQ PARENDER abrir_ambito_funcion bloque cerrar_ambito'
    nombre = p[2]
    ...
    p[0] = FuncDecl(nombre, [], p[6], linea, col)

def p_funcion_sin_params(p):
    'funcion : FUNCION tipo IDENTIFICADOR PARENIZQ PARENDER abrir_ambito_funcion bloque cerrar_ambito'
    retorno = p[2]
    nombre = p[3]
    linea, col = _line(p,3), _col(p,3)
    tabla.agregar_simbolo(
    ...
    p[0] = FuncDecl(nombre, [], p[7], linea, col)
...
# ------------------------------------------------------------
# expresiones (incluye comparaciones)
# ------------------------------------------------------------
def p_expresion(p):
